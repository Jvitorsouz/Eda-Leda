...
public class BST {
    ...
    public int somaIrmaos() {
        return somaIrmaos(this.root);
    }

    private int somaIrmaos(Node current){
        if(current == null) return 0;
        if(current.isLeaf()) return 0;
        if(current.hasOnlyLeftChild()){
            return somaIrmaos(current.left);
        }else if(current.hasOnlyRightChild()){
            return somaIrmaos(current.right);
    }   else{
            return current.right.value + current.left.value + somaIrmaos(current.left) + somaIrmaos(current.right);
        }
    }
